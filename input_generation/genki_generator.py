...
def generate_dataset(image_size, genki_path, dlib_shape_predictor_path, save_path):
    file_images = open(genki_path + 'Subsets/GENKI-4K/GENKI-4K_Images.txt', 'r')
    file_labels = open(genki_path + 'Subsets/GENKI-4K/GENKI-4K_Labels.txt', 'r')
    raw_images = []
    raw_labels = []
    ...
    while True:
        ...
        raw_images.append(cv2.imread(genki_path + "files/" + line))
    ...
    file = open(save_path + 'genki_data_' + str(image_size) + '.pkl', 'wb')
    pickle.dump(input_images, file, protocol=4)
    pickle.dump(input_labels, file, protocol=4)
    file.close()

...
def get_full_save_path(image_size, save_path, **kwargs):
    return save_path + "CNN_" + str(kwargs["dataset"]) + "_" + str(image_size) + "_" + str(
        kwargs["layer_nb"]) + "_" + str(
        kwargs["kernel_nb"]) + "_" + str(kwargs["kernel_size"]) + "_" + str(kwargs["dropout"]) + "_" + str(
        kwargs["contrast"]) + "_" + str(kwargs["flip"]) + "_" + str(kwargs["translation_x"]) + "_" + str(
        kwargs["translation_y"])


##
def train_CNN(dataset_path, image_size, save_path, **kwargs):
    layer_nb = kwargs["layer_nb"]
    kernel_nb = kwargs["kernel_nb"]
    ...
    model.add(Conv2D(kernel_nb, (kernel_size, kernel_size), activation='relu',
